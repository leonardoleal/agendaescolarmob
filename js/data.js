...
var WebSqlStore = function(successCallback, errorCallback) {
    ...
    this.initializeDatabase = function(successCallback, errorCallback) {
        ...
        this.db.transaction(
                function(tx) {
                    ...
                    self.createMessageTable(tx);
                    self.createReplyTable(tx);
    ...
    this.createUserTable = function(tx) {
        ...
        var sql = "CREATE TABLE IF NOT EXISTS user (idUsuario INTEGER, usuario TEXT, senha TEXT, token TEXT, ultimoLogin TEXT);";
    ...
    this.createMessageTable = function(tx) {
        tx.executeSql('DROP TABLE IF EXISTS mensagens');
        var sql = "create table if not exists mensagens (idMensagem INTEGER, idUsuario INTEGER, assunto TEXT, mensagem TEXT, dataEnvio DATE, horaEnvio TIME, professor TEXT);";
        tx.executeSql(sql, null,
                function() {
                    console.log('Create table success');
                },
    ...
    this.createReplyTable = function(tx) {
        tx.executeSql('DROP TABLE IF EXISTS respostas');
        var sql = "create table if not exists respostas (idMensagem INTEGER, idUsuario INTEGER, resposta TEXT, dataEnvio DATE, horaEnvio TIME);";
        tx.executeSql(sql, null,
                function() {
                    console.log('Create table success');
                },
    ...
    this.findByLastLogin = function(callback) {
        this.db.transaction(
            function(tx) {

                var sql = "SELECT * FROM user ORDER BY ultimoLogin DESC";
